...
create view location_details as select department_name, manager_id,location_id from department_details where department_name = 'Shipping'; 
...
create view salaray_range as select employee_id, first_name, last_name, job_id, salary from employee_details
...
create view pattern_matching  as select employee_id, first_name, job_id, salary from employee_details where first_name like '%l';
...
select emp.manager_id, d.department_id from employee_details emp 
join department_details d on emp.department_id = d.department_id;
...
use inclass;
create view SLR_Focus as select `SERIAL NO.`,SOP, LOR, RESEARCH from admission_predict where Research = 1;

# Q.13 Create and append a new column "SOP_LOR_AVG" to the view "SLR_Focus".
alter view SLR_Focus as select `SERIAL NO.`, SOP, LOR, RESEARCH, (SOP + LOR)/2 as New_Score
...
-- Session - User2
insert into bank_customer values(123009,'Report','99-bechkingam','CA', 1867888950);
...
update bank_account set balance_amount = balance_amount +0.03*balance_amount where customer_id = 123001;
commit;
# After transaction of session 1 is completed with commit, below two transactions can be done at a time. 
