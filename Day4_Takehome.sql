...
select bad.Account_Number, sum(Transaction_amount) as F_t  
from bank_account_details bad join bank_account_transaction bat on bad.Account_Number = bat.Account_Number  
where weekday(bat.Transaction_Date) = 4 and Account_type = 'Credit card'
group by bad.Account_type 
union
select bad.Account_Number, sum(Transaction_amount) as O_t 
from bank_account_details bad join bank_account_transaction bat on bad.Account_Number = bat.Account_Number  
where weekday(bat.Transaction_Date) in (0,1,2,3,5,6) and Account_type = 'Credit card'
group by bad.Account_type;

# Question 2:
# 2) Show the details of credit cards along with the aggregate transaction amount during holidays  and non holidays. 
select bad.Account_type, bad.Account_Number, 
case when bat.Transaction_Date = bh.Holiday then sum(Transaction_amount) else null end as 'H_agg'
from bank_account_details bad join bank_account_transaction bat on bad.Account_Number = bat.Account_Number 
cross join bank_holidays bh on bat.Transaction_Date = bh.Holiday where bad.Account_type = 'Credit card'
union
select bad.Account_type, bad.Account_Number, case when bat.Transaction_Date != bh.Holiday then sum(Transaction_amount) else null end as 'NH_agg'
from bank_account_details bad join bank_account_transaction bat on bad.Account_Number = bat.Account_Number 
cross join bank_holidays bh on bat.Transaction_Date = bh.Holiday where bad.Account_type = 'Credit card';
...
select bat.Transaction_Date, 'Happy Holiday' as Message, bh.Holiday from bank_account_transaction bat 
cross join bank_holidays bh on bat.Transaction_Date =  bh.Holiday where month(Transaction_Date) = 3 and bat.Transaction_Date = bh.Holiday;  

# Question 4:
# 4) Calculate the Bank accrued interest with respect to their RECURRING DEPOSITS  for any deposits older than 30 days . 
select bat.Account_Number, bat.Transaction_amount, (bat.Transaction_amount * 0.07) as Accured_Interest
from bank_account_relationship_details a join bank_account_relationship_details b on a.Account_Number= b.Linking_Account_Number
join bank_account_transaction bat on b.Account_Number = bat.Account_Number where b.Account_type like '%recurr%'
group by b.Account_type having datediff(curdate(),bat.Transaction_Date) > 30;
...
select a.Account_Number, a.Account_type, b.Linking_Account_Number, b.Account_type, count(c.Transaction_amount) as count
from bank_account_details a join bank_account_relationship_details b 
on a.Account_Number = b.Linking_Account_Number 
join bank_account_transaction c on b.Account_Number = c.Account_Number 
where b.Account_type like '%credit%' and a.Account_type like '%Savings%'
...
select a.Account_Number, a.Account_type, b.Linking_Account_Number, b.Account_type, count(c.Transaction_amount) as count 
from bank_account_details a join bank_account_relationship_details b on a.Account_Number = b.Linking_Account_Number  
join bank_account_transaction c on b.Account_Number = c.Account_Number 
where b.Account_type like '%add%' and a.Account_type like '%Credit%'
group by a.Account_Number, a.Account_type, b.Linking_Account_Number, b.Account_type 
having count(c.Transaction_amount) >= 1;
...
select be.customer_id, be.customer_name 
from bank_customer_export be natural inner join bank_customer bc;
